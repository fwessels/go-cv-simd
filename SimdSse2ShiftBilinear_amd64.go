...
//
func SimdSse2ShiftBilinear(src, bkg gocv.View, dst gocv.View) {

	shiftX, shiftY := float64(0.0), float64(0.0)
	cropLeft, cropTop, cropRight, cropBottom := uint64(0), uint64(0), uint64(255), uint64(255)

	_SimdSse2ShiftBilinear(src.GetData(), uint64(src.GetStride()), uint64(src.GetWidth()), uint64(src.GetHeight()), uint64(gocv.ChannelCount(src.GetFormat())),
